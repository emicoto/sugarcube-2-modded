...
var Engine = (() => {
         ...
         if (markup) {
            ...
            $elems.find("[data-init-passage]").each((i, el) => {
               ...
               if (el.hasAttribute("data-passage")) {
                  throw new Error(
                     `"StoryInterface" element <${el.nodeName.toLowerCase()} data-init-passage="${passage}"> must not contain a "data-passage" content attribute`
                  );
               }

               if (el.firstElementChild !== null) {
                  throw new Error(
                     `"StoryInterface" element <${el.nodeName.toLowerCase()} data-init-passage="${passage}"> contains child elements`
                  );
               }
            ...
            $elems.find("[data-passage]").each((i, el) => {
               if (el.id === "passages") {
                  throw new Error(
                     `"StoryInterface" element <${el.nodeName.toLowerCase()} id="passages"> must not contain a "data-passage" content attribute`
                  );
               }
               ...
               if (el.firstElementChild !== null) {
                  throw new Error(
                     `"StoryInterface" element <${el.nodeName.toLowerCase()} data-passage="${passage}"> contains child elements`
                  );
               }
            ...
            $elems.append(
               '<div id="story-header" class="bar-extend"></div><div id="main"><div id="story" role="main" class="bar-extend"><div id="passages" aria-live="polite"></div></div></div><div id="story-footer" class="bar-extend"></div>'
            );
         }
   ...
   function engineStart() {
      ...
      /*
		...
		*/
      jQuery.event.trigger({ type: ":storyinit" });
      if (Story.has("StoryInit")) {
         try {
            const debugBuffer = Wikifier.wikifyEval(
   ...
   function enginePlay(title, noHistory) {
      if (DEBUG) {
         console.log(
            `[Engine/enginePlay(title: "${title}", noHistory: ${noHistory})]`
         );
      }
      ...
      //check if passage exists in current language
      ...
      if (!noHistory && !passage.tags.includes("system")) {
         if (Config.debug) console.log("create", passage);
      ...
      // Render the passage into the passage element. if the language version  exists, render that instead
      else {
         if (passage_lan) {
            passageEl.appendChild(passage_lan.render());
         } else {
            passageEl.appendChild(passage_sec.render());
         }

         //set the temporary state to true, so the system know we have a language version
         State.temporary.hasLang = true;
      }
      ...
      //Era system will try to find language json file and load it at this point
   ...
   function engineDisplay(title, link, option) {
      ...
      switch (option) {
         ...
         default:
            throw new Error(
               `Engine.display option parameter called with obsolete value "${option}"; please notify the developer`
            );
      }
