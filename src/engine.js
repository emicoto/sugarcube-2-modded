...
var Engine = (() => {
  ...
  function engineInit() {
    ...
    (() => {
      ...
      const markup =
        Story.has("StoryInterface") && Story.get("StoryInterface").text.trim();
  ...
  function enginePlay(title, noHistory) {
    ...
    if (Story.has(checkBefore)) {
      try {
        passageBeforeOutput = Wikifier.wikifyEval(Story.get(checkBefore).text);
      } catch (ex) {
        console.error(ex);
    ...
    if (Story.has(checkAfter)) {
      try {
        passageAfterOutput = Wikifier.wikifyEval(Story.get(checkAfter).text);
      } catch (ex) {
        console.error(ex);
