...
var UI = (() => {
  ...
  function uiAssembleLinkList(passage, listEl) {
    ...
    try {
      ...
      const errors = [...frag.querySelectorAll(".error")].map((errEl) =>
        errEl.textContent.replace(errorPrologRegExp, "")
      );
  ...
  function uiOpenAlert(message, /* options, closeFn */ ...args) {
    jQuery(Dialog.setup(L10n.get("alertTitle"), "alert")).append(
      `<p>${message}</p><ul class="buttons">` +
        `<li><button id="alert-ok" class="ui-close">${L10n.get([
          "alertOk",
          "ok",
        ])}</button></li>` +
        "</ul>"
  ...
  function uiBuildAutoload() {
    ...
    jQuery(Dialog.setup(L10n.get("autoloadTitle"), "autoload")).append(
      ...
      `<p>${L10n.get("autoloadPrompt")}</p><ul class="buttons">` +
        `<li><button id="autoload-ok" class="ui-close">${L10n.get([
          "autoloadOk",
          "ok",
        ])}</button></li>` +
        `<li><button id="autoload-cancel" class="ui-close">${L10n.get([
          "autoloadCancel",
          "cancel",
        ])}</button></li>` +
        "</ul>"
  ...
  function uiBuildJumpto() {
    ...
    if (!list.hasChildNodes()) {
      jQuery(list).append(
        `<li><a><em>${L10n.get("jumptoUnavailable")}</em></a></li>`
      );
    }
  }
    ...
    jQuery(Dialog.setup(L10n.get("restartTitle"), "restart"))
      .append(
        ...
        `<p>${L10n.get("restartPrompt")}</p><ul class="buttons">` +
          `<li><button id="restart-ok">${L10n.get([
            "restartOk",
            "ok",
          ])}</button></li>` +
          `<li><button id="restart-cancel" class="ui-close">${L10n.get([
            "restartCancel",
            "cancel",
          ])}</button></li>` +
          "</ul>"
  ...
  function uiBuildSaves() {
    const savesAllowed =
      typeof Config.saves.isAllowed !== "function" || Config.saves.isAllowed();
    ...
    const fileOk =
      Has.fileAPI && (Config.saves.tryDiskOnMobile || !Browser.isMobile.any());
    ...
    if (savesOk || fileOk) {
      ...
      if (fileOk) {
        ...
        $btnBar.append(
          createActionItem("import", null, L10n.get("savesLabelImport"), () =>
  ...
  function uiBuildSettings() {
    ...
    Setting.forEach((control) => {
      if (control.type === Setting.Types.Header) {
        ...
        $header
          ...
          .append($heading)
          .appendTo($dialogBody);
        $heading.attr("id", `header-heading-${id}`).wiki(name);
    ...
    $dialogBody
      .append(
        '<ul class="buttons">' +
          `<li><button id="settings-ok" class="ui-close">${L10n.get([
            "settingsOk",
            "ok",
          ])}</button></li>` +
          `<li><button id="settings-reset">${L10n.get(
            "settingsReset"
          )}</button></li>` +
          "</ul>"
      )
    ...
    Object.defineProperties(
      {},
      {
		...
		*/
        ...
        addClickHandler: {
          value: (...args) => Dialog.addClickHandler(...args),
        },
